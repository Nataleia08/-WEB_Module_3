...
def translate_name_file(name) -> str:
    ...
    "f", "h", "ts", "ch", "sh", "sch", "", "y", "", "e", "yu", "ya", "je", "i", "ji", "g", "A", "B", "V", "G", "D", "E",
    "E", "J", "Z", "I", "J", "K", "L", "M", "N", "O", "P", "R", "S", "T", "U",
...
def sorting(path: Path, list_path) -> list:
    ...
    if not len(listdir(path)):  # -----Видалення порожніх папок--------------
        ...
        for i_path in new_path_name.iterdir():
            if (i_path.name == "video") or (i_path.name == "audio") or (i_path.name == "archives") or (
                    i_path.name == "documents") or (i_path.name == "images") or (i_path.name == "other"):
                continue
            if i_path.is_dir():
                thread = Thread(target=sorting, args=(i_path, list_path))
                thread.start()
                threads.append(thread)
                print("Start new block threading. Main thread #", len(threads))
            else:
                i_new = normalize(i_path)
                i_roz = i_new.suffix.removeprefix(".")
                if i_roz in list_images_path_small:
                    thread = Thread(target=move_files, args=(i_new, 4, list_path))
                    thread.start()
                    threads.append(thread)
                    print("Start threading #", len(threads))
                elif i_roz in list_video_path_small:
                    thread = Thread(target=move_files, args=(i_new, 0, list_path))
                    thread.start()
                    threads.append(thread)
                    print("Start threading #", len(threads))
                elif i_roz in list_docum_path_small:
                    thread = Thread(target=move_files, args=(i_new, 3, list_path))
                    thread.start()
                    threads.append(thread)
                    print("Start threading #", len(threads))
                elif i_roz in list_audio_path_small:
                    thread = Thread(target=move_files, args=(i_new, 2, list_path))
                    thread.start()
                    threads.append(thread)
                    print("Start threading #", len(threads))
                    ...
                    try:
                        thread = Thread(target=unpack_archive, args=(moved_path, arh_p))
                        thread.start()
                        threads.append(thread)
                        print("Start threading #", len(threads))
                    except OSError as e:
                    ...
                    try:
                        thread = Thread(target=move, args=(i_new, list_path[0][5]))
                        thread.start()
                        threads.append(thread)
                        print("Start threading #", len(threads))
                    except OSError as e:
